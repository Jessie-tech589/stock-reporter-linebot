...
def weather(city, lat, lon):
    try:
        ...
        if not loc_data or not isinstance(loc_data, list) or not loc_data[0] or "Key" not in loc_data[0]:
            raise ValueError("AccuWeather location data missing or invalid.")
        key = loc_data[0]["Key"]
        ...
        wx_res = requests.get(url_wx, timeout=8)
        wx = wx_res.json()
        if not wx or not isinstance(wx, list) or not wx[0]:
            raise ValueError("AccuWeather current conditions data missing.")
        
        temp = wx[0].get('Temperature', {}).get('Metric', {}).get('Value')
        realfeel = wx[0].get('RealFeelTemperature', {}).get('Metric', {}).get('Value')
        wxtext = wx[0].get('WeatherText')
        
        if temp is None or realfeel is None or wxtext is None:
            raise ValueError("AccuWeather temperature, realfeel, or weather text missing.")
    ...
    try:
        ...
        main_data = js.get("main", {})
        weather_data = js.get("weather", [])
        
        temp = main_data.get("temp")
        feels = main_data.get("feels_like")
        desc = weather_data[0].get("description") if weather_data else None
        cityname = js.get("name", city)

        if temp is None or feels is None or desc is None:
            raise ValueError("OpenWeatherMap data missing temperature, feels_like, or description.")
...
def fx():
    ...
    try:
        ...
        js = requests.get(url, timeout=8).json()
        rate = js.get("Realtime Currency Exchange Rate", {}).get("5. Exchange Rate")
        if rate:
            return f"💱 USD/TWD: {float(rate):.2f}\n來源: AlphaVantage"
        else:
            raise ValueError("AlphaVantage rate data missing.")
    except Exception as e:
...
def cal():
    try:
        if not GOOGLE_CREDS_JSON_B64:
            logging.warning("[CAL-ERR] GOOGLE_CREDS_JSON_B64 is not set.")
            return "行事曆查詢失敗：憑證未設定"
        
        try:
            decoded_creds_bytes = base64.b64decode(GOOGLE_CREDS_JSON_B64)
        except Exception as e:
            logging.warning(f"[CAL-ERR] Base64 decode failed: {e}")
            return "行事曆查詢失敗：憑證解碼失敗"

        try:
            info = json.loads(decoded_creds_bytes.decode('utf-8'))
        except (json.JSONDecodeError, UnicodeDecodeError) as e:
            logging.warning(f"[CAL-ERR] JSON or UTF-8 decode failed: {e}")
            return "行事曆查詢失敗：憑證JSON解析失敗"
...
def us():
    ...
    return "🇺🇸 美股行情 (YFinance)\n" + "\n".join(idx_lines + focus_lines) # Combined for both summaries
...
def traffic(label):
    ...
    url = (
        ...
        f"&key={Maps_API_KEY}&departure_time=now&language=zh-TW"
    )
    try:
        r = requests.get(url, timeout=8)
        js = r.json()
...
def push(message):
    ...
    except LineBotApiError as e: # Explicitly catch LineBotApiError
        logging.error(f"[LineBot] 推播失敗：{e}")
    except Exception as e:
        logging.error(f"[LineBot] 推播失敗 (非Line API錯誤)：{e}")
...
def morning_briefing():
    ...
    try:
        # 修正函數名稱並移除未定義的函數
        weather_info = weather("中山區", *LOCATION_COORDS["中山區"])
        calendar_info = cal()
        fx_info = fx()
        us_market_info = us() # us() 函數已包含美股大盤和個股資訊
        ...
        # 修正 USER_ID 為 LINE_USER_ID，並一次性發送
        try:
            line_bot_api.push_message(LINE_USER_ID, TextSendMessage(text=full_message))
        except LineBotApiError as e:
            logging.error(f"[LinePushError] LINE API 推播失敗（月用量或頻率限制）：{e}")
        except Exception as e:
            logging.error(f"[LinePushError] 推播失敗：{e}")
