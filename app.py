...
def get_weather(location):
    ...
    try:
        ...
        elements = {e['elementName']: e['time'][0]['elementValue'][0]['value'] for e in weather['weatherElement']}
        return (
            f"☀️ {name}天氣\n\n"
            f"🌡️ 溫度: {elements.get('MinT', '')}-{elements.get('MaxT', '')}°C\n"
            f"💧 降雨機率: {elements.get('PoP12h', '')}%\n"
            f"☁️ 天氣: {elements.get('Wx', '')}\n"
            f"🌡️ 舒適度: {elements.get('CI', '')}\n\n"
            f"資料來源: 中央氣象署"
        )
    except Exception as e:
        return f"❌ {location}天氣\n\n取得資料失敗"
...
def get_us_stock_info(symbol):
    try:
        ...
        symbol_icon = "📈" if change > 0 else ("📉" if change < 0 else "📊")
        return (
            f"{symbol_icon} 美股 {symbol}\n\n"
...
def get_exchange_rate():
    try:
        ...
        reply = (
            f"💱 匯率\n\n"
            f"1美元 ➜ {rates.get('TWD', 0):.2f} 台幣\n"
            f"1美元 ➜ {rates.get('JPY', 0):.2f} 日圓\n"
            f"1美元 ➜ {rates.get('CNY', 0):.2f} 人民幣\n"
            f"1美元 ➜ {rates.get('HKD', 0):.2f} 港幣\n"
            f"1美元 ➜ {rates.get('GBP', 0):.2f} 英磅"
        )
        return reply
    except Exception as e:
...
def get_fuel_price():
    try:
        url = "https://vipmbr.cpc.com.tw/OpenData.aspx?SN=8A2E8C1D1DF9494C"
        res = requests.get(url, timeout=10)
        res.encoding = 'utf-8'
        text = res.text
...
SCHEDULED_MESSAGES = [
    ...
    {"time": "07:30", "message": "us_market_report", "days": "23456"}  # 美股早上看
]
...
def get_us_market_report():
    ...
    messages = [get_us_stock_info(symbol) for symbol in symbols]
    return "🌎 美股行情報告\n\n" + "\n\n".join(messages)
...
def handle_message(event):
    ...
    if msg in CUSTOM_ROUTES:
        ...
        reply = get_fuel_price()
    elif msg == "匯率":
        reply = get_exchange_rate()
    else:
        reply = (
            "👋 功能：\n"
            "• 🚦「家到公司」「公司到家」「公司到郵局」查機車路線\n"
            "• 🌤️「天氣區名」\n"
            "• 📰「新聞關鍵字」\n"
            "• 📈「台股 名稱」 / 「美股 名稱」\n"
            "• 📅「行事曆」查 Google 行程\n"
